...
#         self.bias = nn.Parameter(torch.one(1))
#     def forward(self, x):
#         mean = x.mean(dim=-1, keepdim=True)
#         std = x.std(dim=-1, keepdim=True)
#         return self.alpha * (x  - mean)/(std + self.eps) +  self.bias
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, features: int, self_attention_block: MultiheadAttentionBlock, feed_forward_block: FeedForwardblock, dropout: float) -> None:
        ...
        self.residual_connections = nn.ModuleList([Residual_Connetion(features, dropout) for _ in range(2)])
...
class DecoderBlock(nn.Module):
    def __init__(self,features:int,self_attention_block:MultiheadAttentionBlock, cross_attention_block:MultiheadAttentionBlock, feed_forward_block:FeedForwardblock, dropout:float):
        ...
        self.feed_forward_block =  feed_forward_block
        self.residual_connections = nn.ModuleList([Residual_Connetion(features, dropout) for _ in range(3)])
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int=512, N: int=6, h: int=8, dropout: float=0.1, d_ff: int=2048) -> Transformer:
    ...
    # Create the positsrcional encoding layers
